...
-- supongo que no hace falta respetar los nombres de los parametros
------------------ EJERCICIO 1 ------------------
...
ciudadesConectadas vuelos ciudad = eliminarRepeticiones (ciudadesConectadasConRepeticiones vuelos ciudad)
    -- Llamo a ciudadesConectadasConRepeticiones y luego elimino las repeticiones con eliminarRepeticiones

ciudadesConectadasConRepeticiones :: AgenciaDeViajes -> Ciudad -> [Ciudad] -- Devuelve las ciudades conectadas con repeticiones
ciudadesConectadasConRepeticiones [] _ = []
ciudadesConectadasConRepeticiones ((origen, destino, _):vuelos) ciudad
    | ciudad == origen = destino:ciudadesConectadasConRepeticiones vuelos ciudad -- Si es igual al origen, agrego el destino como conexion
    | ciudad == destino = origen:ciudadesConectadasConRepeticiones vuelos ciudad -- Si es igual al destino, agrego el origen como conexion
    | otherwise = ciudadesConectadasConRepeticiones vuelos ciudad -- Ignoro el vuelo actual y sigo con la lista

eliminarRepeticiones :: [Ciudad] -> [Ciudad] -- Elimina las repeticiones de una lista
eliminarRepeticiones [] = []
eliminarRepeticiones (ciudad:ciudades)
    | elem ciudad ciudades = eliminarRepeticiones ciudades -- Si la ciudad esta en la lista, la ignoro
    | otherwise = ciudad:eliminarRepeticiones ciudades -- Si no esta, la agrego y sigo con la lista
...
aux = [("Rosario", "Córdoba", 10.0), ("Salta", "Chapadmalal", 19.0),("Córdoba","Rosario",9.0), ("Salta", "Chapadmalal", 19.0)] 
...
sePuedeLlegarConUnaEscala _ [] _ = False
...
obtenerDuracionMinima (duracion:ds)
    | duracion < head ds && duracion /= 0 = obtenerDuracionMinima (duracion:tail ds)
